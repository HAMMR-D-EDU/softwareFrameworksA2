...
router.post('/:groupId/interest', async (req, res) => {
  try {
    ...
    // Notify group admins in real-time via Socket.IO
    notifyGroupAdmins(groupId, {
      type: 'new_interest',
      groupId,
      ...
      message: `${user?.username || 'A user'} wants to join ${group.name}`
    });
...
router.post('/:groupId/interests/:interestId/approve', async (req, res) => {
  try {
    ...
    // Add user to group members (not as admin - must be promoted explicitly)
    await groupsCollection.updateOne(
      { id: groupId },
...
router.get('/:groupId/channels', async (req, res) => {
  try {
    ...
    // All group members can see all channels in the group
    // (They can see the channels but may not be able to access them)
